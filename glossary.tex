...
\newduneabbrev{afc}{AFC}{AMC FMC Carrier}{AMC FMC carrier design from ohwr.org}

% Small form-factor pluggable transceiver
\newduneabbrev{sfp}{SFP}{Small Form-Factor Pluggable} {A hot-pluggable transceiver for carrying Gigabit/s serial signals. The transport medium is usually an optical fiber}

\newduneabbrev{ccm}{CCM}{Configuration Control and Management Control} {A system to manage and record changes to the experiment's configuration.}
...
% UTC
\newduneabbrev{utc}{UTC}{Coordinated Universal Time}{Coordinated Universal Time is the primary time standard by which the world regulates clocks and time.}

\newduneabbrev{dts-tm}{timing master}{DTS master}{Originator of master timing signals in (Proto)DUNE}

\newduneabbrev{dts-ep}{timing endpoint}{DTS endpoint}{Recipient of timing signals from the timing master in (Proto)DUNE}
% 
\newduneabbrev{irig}{IRIG}{Inter-Range Instrumentation Group}{An amplitude modulated carrier encoded with UTC time. Current standard is IRIG 200-04}

%gps interface board
\newduneabbrev{gib}{GIB}{GPS Interface Board}{Interface to the GPS receiver} 
 
%utca interface board
\newduneabbrev{mib}{MIB}{\gls{utca} Interface Board}{Interface between the GIB and $u$TCA crate, as well as AMCs} 

%fib interface board
\newduneabbrev{fib}{FIB}{Fibre Interface Board}{interface to the timing endpoints} 

%mch
\newduneabbrev{mch}{MCH}{$\mu$TCA Carrier Hub}{uTCA crate controller} 

%\newduneabbrev{eti}{ETI}{External Trigger Interface}{Units receiving external signals into the \dword{spdts}}
\newduneabbrev{hsi}{HSI}{Hardware Signal Interface}{Units receiving external signals into the \dword{spdts}}

\newduneabbrev{ctb}{CTB}{Central Trigger Board}{The unit that receives hardware trigger signals at \dword{pdsp1} and sending a serialized data stream to the \dword{spts}}

\newduneabbrev{gpsdo}{GPSDO}{GPS Disciplined Oscillator}{A crystal oscillator steered by GPS}

\newduneabbrev{oxco}{OXCO}{Oven Controlled Crystal Oscillator}{A quartz crystal, housed in a temperature-controlled chamber, so that a constant temperature is maintained, in order to prevent changes in the frequency due to variations in ambient temperature}
...
%clock data recovery
\newduneabbrev{cdr}{CDR}{Clock-Data Recovery}{Clock-data recovery}
...
\newduneabbrev{pdsp1}{ProtoDUNE SP I}{ProtoDUNE Single Phase I}{The first phase of the ProtoDUNE Single Phase experiment at CERN.}

\newduneabbrev{pdsp2}{ProtoDUNE SP II}{ProtoDUNE Single Phase II}{The second phase of the ProtoDUNE Single Phase experiment at CERN.}

\newduneabbrev{dts}{DTS}{Dune Timing System}{The timing and synchronization system for DUNE. }

\newduneabbrev{spts}{PDSPTS}{ProtoDUNE Single Phase Timing System}{A timing and synchronization system deployed at ProtoDUNE SP I. It distributes a clock and synchronization signals, but places considerably less burden on the end-points.}

\newduneabbrev{spdts}{DTS-SP}{DUNE Timing System-Single Phase}{A timing and synchronization system which will be developed and deployed at ProtoDUNE SP II, and eventually DUNE. It will distribute clock and synchronisations signal to \single detector modules only.}

\newduneabbrev{dpdts}{DTS-DP}{DUNE Timing System-Dual Phase}{A timing and synchronization system which is deployed at ProtoDUNE DP, and eventually DUNE. It will distribute clock and synchronisations signal to \dual detector modules only.}
...
\newduneword{hpc}{HPC}{high pin count}
...
\newduneword{ipbb}{IPBB}{IPBus Build, a system for building an FPGA configuration file from the source code. https://github.com/ipbus/ipbb}

\newduneword{ipbus}{IPBus}{IPBus, Firmware that implements a reliable high-performance control link for particle physics electronics, based on the IPbus protocol https://ipbus.web.cern.ch}